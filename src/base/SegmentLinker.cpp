...
SegmentLinker::clearRefreshStatuses()
{
    ...
    for (itr = m_linkedSegmentParamsList.begin();
        ...
        if (!linkedSegToUpdate->haveRefreshStatus(refreshStatusId))
            // Shouldn't ever happen, but seeing in lilypond_export_test
            return;
...
{
    m_linkedSegment->deleteRefreshStatusId(m_refreshStatusId);
}
